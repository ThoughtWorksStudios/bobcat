;(function() {
  ...
  Generator.prototype.one = function generateSingle(parentId) {
    var id = this.fields["$id"].value(), result = {"$id": id};

    if (parentId) result["$parent"] = parentId;

    var keys = Object.keys(this.fields);

    for (var i = 0, len = keys.length, name, field; i < len; ++i) {
      name = keys[i], field = this.fields[name];
      if (!result.hasOwnProperty(name)) {
        result[name] = field.value(id);
      }
    }
  ...
  function Field(config) {
    ...
    this.value = function generateValue(parentId) {
      if (!config || !config.countRange) {
        return this.one(parentId);
      }
      var result = [];
      for (var i = 0, count = config.countRange.count(); i < count; ++i) {
        result.push(this.one(parentId));
      }
      return result;
    }
  }
  ...
  function ReferenceField(config) {
    ...
    this.one = function resolveValueFromParent(parentId) { return resolved.value(parentId); };
  }

  function EntityField(config) {
    Field.call(this, config);
    this.type = "entity";
    this.one = function nestedEntity(parentId) {
      return config.entity.one(parentId);
    };
  }
