...
func TestParsesBasicGenerationStatement(t *testing.T) {
	args := NodeSet{Node{Kind: "literal-int", Value: 1, Ref: NewLocation("", 1, 10, 9)}}
	genBird := testGenEntity("Bird", NewLocation("", 1, 1, 0), NodeSet{}, args)
	testRoot := testRootNode(NodeSet{genBird})
	actual, err := Parse("", []byte("generate(1, Bird)"))
	AssertNil(t, err, "Didn't expect to get an error: %v", err)
	AssertEqual(t, testRoot.String(), actual.(Node).String())
}

func TestCanParseMultipleGenerationStatements(t *testing.T) {
	arg := Node{Kind: "literal-int", Value: 1, Ref: NewLocation("", 1, 10, 9)}
	genBird := testGenEntity("Bird", NewLocation("", 1, 1, 0), NodeSet{}, NodeSet{arg})
	arg.Ref = NewLocation("", 2, 10, 27)
	bird2Gen := testGenEntity("Bird2", NewLocation("", 2, 1, 18), NodeSet{}, NodeSet{arg})
	bird2Gen.Name = "Bird2"
...
func TestCanOverrideFieldInGenerateStatement(t *testing.T) {
	...
	testRoot := testRootNode(NodeSet{genBird})
	actual, err := Parse("", []byte("generate(1, Bird { name \"birdie\" })"))
	AssertNil(t, err, "Didn't expect to get an error: %v", err)
	AssertEqual(t, testRoot.String(), actual.(Node).String())
}
...
func TestCanOverrideMultipleFieldsInGenerateStatement(t *testing.T) {
	...
	testRoot := testRootNode(NodeSet{genBird})
	actual, err := Parse("", []byte("generate(1, Bird { name \"birdie\", age integer(1,2) })"))
	AssertNil(t, err, "Didn't expect to get an error: %v", err)
	AssertEqual(t, testRoot.String(), actual.(Node).String())
}

func TestParsedBothBasicEntityAndGenerationStatement(t *testing.T) {
	args := NodeSet{Node{Kind: "literal-int", Value: 1, Ref: NewLocation("", 2, 10, 21)}}
...
func TestShouldGiveErrorForUnknownFieldTypes(t *testing.T) {
	specs := map[string]string{
		"generate(1, t { e eek })": `no match found, expected: "-", "0", "\"", "\\0", "date", "decimal", "dict", "false", "integer", "null", "string", "t"i, "true", [ \t\r\n], [0-9] or [1-9]`,
...
func TestShouldGiveErrorWhenNoCountIsGivenToGenerate(t *testing.T) {
	expectedErrMessage := `no match found, expected: "(" or [ \t\r\n]`
...
func TestEntityNameMustBeAlphaNumericAndStartWithALetter(t *testing.T) {
	specs := map[string]string{
		...
		"generate(1, $eek)": `no match found, expected: "date", "decimal", "def", "dict", "false", "generate", "integer", "null", "string", "true", [ \t\r\n] or [a-z_]i`,
		"generate(1, eek$)": `no match found, expected: ")", "{", [ \t\r\n] or [a-z0-9_]i`,
