...
func (g *Generator) WithStaticField(fieldName string, fieldValue interface{}) error {
	if _, ok := g.fields[fieldName]; ok {
		return NewWarningError("already defined field: %s", fieldName)
	}
...
func (g *Generator) WithField(fieldName, fieldType string, fieldOpts interface{}) error {
	if fieldOpts == nil {
		return NewFatalError("FieldOpts are nil for field '%s', this should never happen!", fieldName)
	}
	...
	case "string":
		if ln, ok := fieldOpts.(int); ok {
			...
			return NewFatalError("expected field options to be of type 'int' for field %s (%s), but got %v",
	...
	case "integer":
		if bounds, ok := fieldOpts.([2]int); ok {
			...
			if max < min {
				return NewFatalError("max %v cannot be less than min %v", max, min)
			}

			g.fields[fieldName] = &IntegerField{min: min, max: max}
		} else {
			return NewFatalError("expected field options to be of type '(min:int, max:int)' for field %s (%s), but got %v", fieldName, fieldType, fieldOpts)
		}
	case "decimal":
		if bounds, ok := fieldOpts.([2]float64); ok {
			...
			return NewFatalError("expected field options to be of type '(min:float64, max:float64)' for field %s (%s), but got %v", fieldName, fieldType, fieldOpts)
		}
	case "date":
		if bounds, ok := fieldOpts.([2]time.Time); ok {
			...
			if !field.ValidBounds() {
				return NewFatalError("max %v cannot be before min %v", max, min)
			}
			g.fields[fieldName] = field
		} else {
			return NewFatalError("expected field options to be of type 'time.Time' for field %s (%s), but got %v", fieldName, fieldType, fieldOpts)
		}
	case "dict":
		if dict, ok := fieldOpts.(string); ok {
			g.fields[fieldName] = &DictField{category: dict}
		} else {
			return NewFatalError("expected field options to be of type 'string' for field %s (%s), but got %v", fieldName, fieldType, fieldOpts)
		}
	default:
		return NewFatalError("Invalid field type '%v'", fieldType)
	}
	return nil
}
